...
int main()
{
	...
	for ( i = 1 ; i <= n ; i ++ )
	{
		...
		while(1)
		{
			...
			else if(heap[temp] > heap[temp/2])//parent값보다 현재 child값이 클때 바꿔준다. =>부호만 바꾸면 min heap!
	...
	while(1)
	{
		...
		while(1)
		{
			...
			if((heap[temp] > heap[lch] && heap[temp] > heap[rch]) || (rch > rear && lch > rear) )//RCH,LCH위치가 rear을 벗어나거나, 현재 위치 값이 LCH, RCH보다 클때
			...
			{
				if(heap[lch] > heap[rch] || rch > rear) // RCH위치 범위가 rear을 벗어나거나 LCH가 RCH보다 클때
				{
					swap(&heap[lch], &heap[temp]);//최대값과 LCH값 바꿈.
					temp = lch;//LCH로 이동
					break;
				}
				else if(heap[lch] < heap[rch] || lch < rear)// LCH위치 범위가 rear을 벗어나거나 RCH가 LCH보다 클때
				{
